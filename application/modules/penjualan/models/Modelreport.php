...
class modelReport extends CI_Model {
    ...
    public function arAgingData($fromDate, $toDate, $calculateDate, $custIdGroup = "0", $idDistributor = "0", $idCustomer = "0", $idSales = "0", $dueDate = "0")
    {
        ...
        LEFT JOIN (
            ...
            WHERE DATEDIFF('".$toDate."', a.`due_date`) <= 0
            GROUP BY a.`id`
        ...
        LEFT JOIN (
            ...
            WHERE DATEDIFF('".$toDate."', b.`due_date`) >= 0 AND DATEDIFF('".$toDate."', b.`due_date`) <=30
        ...
        LEFT JOIN (
            ...
            FROM `trans_um_invoice_detail` a 
            JOIN `trans_invoice` b ON b.`id`=a.`id_invoice`
            WHERE DATEDIFF('".$toDate."', b.`due_date`) >= 31 AND DATEDIFF('".$toDate."', b.`due_date`) <= 60
            GROUP BY a.`id_invoice`
        ...
        LEFT JOIN (
            ...
            FROM `trans_um_invoice_detail` a 
            JOIN `trans_invoice` b ON b.`id`=a.`id_invoice`
            WHERE DATEDIFF('".$toDate."', b.`due_date`) >= 61 AND DATEDIFF('".$toDate."', b.`due_date`) <= 90
            GROUP BY a.`id_invoice`
        ...
        LEFT JOIN (
            ...
            FROM `trans_um_invoice_detail` a 
            JOIN `trans_invoice` b ON b.`id`=a.`id_invoice`
            WHERE DATEDIFF('".$toDate."', b.`due_date`) >= 91
            GROUP BY a.`id_invoice`
        ...
        */
        ...
        b.`payment` AS total_payment, 
	    IFNULL(b.`remaining_amount` - b.`payment`, a.`amount`) AS total_remain,
        m.belum_due_date, z.kolom_satu, w.kolom_dua, u.kolom_tiga, v.kolom_empat
        ...
        LEFT JOIN `mst_sales_person` e ON e.`id`=a.`id_sales`
        LEFT JOIN (
            SELECT a.`id` AS id_invoice, IFNULL(SUM(b.`remaining_amount`) - SUM(b.`payment`), SUM(a.`amount`))  AS belum_due_date 
            FROM `trans_invoice` a 
            LEFT JOIN `trans_um_invoice_detail` b ON a.`id`=b.`id_invoice`
        ...
        LEFT JOIN (
	    SELECT a.`id` AS id_invoice, IFNULL(SUM(b.`remaining_amount`) - SUM(b.`payment`), SUM(a.`amount`))  AS kolom_satu 
            FROM `trans_invoice` a 
            LEFT JOIN `trans_um_invoice_detail` b ON a.`id`=b.`id_invoice`
            WHERE DATEDIFF('".$calculateDate."', a.`due_date`) >= 0 AND DATEDIFF('".$calculateDate."', a.`due_date`) <=30
            GROUP BY a.`id`
           
        ) z ON z.`id_invoice` = a.`id`
        LEFT JOIN (
	    
	    SELECT a.`id` AS id_invoice, IFNULL(SUM(b.`remaining_amount`) - SUM(b.`payment`), SUM(a.`amount`))  AS kolom_dua 
            FROM `trans_invoice` a 
            LEFT JOIN `trans_um_invoice_detail` b ON a.`id`=b.`id_invoice`
            WHERE DATEDIFF('".$calculateDate."', a.`due_date`) >=31 AND DATEDIFF('".$calculateDate."', a.`due_date`) <=60
            GROUP BY a.`id`
        ) w ON w.`id_invoice` = a.`id`
        LEFT JOIN (
	    
	    SELECT a.`id` AS id_invoice, IFNULL(SUM(b.`remaining_amount`) - SUM(b.`payment`), SUM(a.`amount`))  AS kolom_tiga 
            FROM `trans_invoice` a 
            LEFT JOIN `trans_um_invoice_detail` b ON a.`id`=b.`id_invoice`
            WHERE DATEDIFF('".$calculateDate."', a.`due_date`) >=61 AND DATEDIFF('".$calculateDate."', a.`due_date`) <=90
            GROUP BY a.`id`
            
        ) u ON u.`id_invoice` = a.`id`
        LEFT JOIN (
            SELECT a.`id` AS id_invoice, IFNULL(SUM(b.`remaining_amount`) - SUM(b.`payment`), SUM(a.`amount`))  AS kolom_empat 
            FROM `trans_invoice` a 
            LEFT JOIN `trans_um_invoice_detail` b ON a.`id`=b.`id_invoice`
            WHERE DATEDIFF('".$calculateDate."', a.`due_date`) >=91 
            GROUP BY a.`id`
        
        ) v ON v.`id_invoice` = a.`id`";

        $sql.=" WHERE a.`due_date` BETWEEN '".$fromDate."' AND '".$toDate."' AND a.`status`='approve'";
    ...
    {
        $sql = "SELECT IF(a.payment_status<>'fullpaid', 'Invoice', 'Paid') AS status_invoice, a.* FROM (
                    ...
                    b.`invoice_number`, b.status, IFNULL(i.id, 0) AS paid, a.`qty` AS qty_penjualan, 
                    c.`point`, c.`point` * a.`qty`  AS bonus, i.`id`, b.payment_status
